...
def log(N, sigma):
    ...
    for i in np.arange(-n2, n2 + 1):
        for j in np.arange(-n2, n2 + 1):
            log[i + n2, j + n2] = (np.power(i, 2) + np.power(j, 2) - 2 * np.power(sigma, 2)) * g[i + n2, j + n2] / (
                        2.0 * 3.1415926 * np.power(sigma, 6) * sumg)
    return log
...
def gam_rem_adp_log(img, thr3=25, thr5=100, thr7=400, sig_log=0.8):
    ...
    if np.sum(imgthr7) >= 0:
        ...
        single7 = signal.convolve2d(imgthr7 * 255, s / np.sum(s),
                                    mode='same') < 30  # boxcar smoothing plus threshold to identify single pixel spots
    ...
    for i in range(n5):
        img_adp[index[0][i], index[1][i]] = np.median(
                img[index[0][i] - 2:index[0][i] + 3, index[1][i] - 2:index[1][i] + 3])

    index = np.nonzero(imgthr7)
    n7 = np.shape(index)[1]
    for i in range(n7):
        img_adp[index[0][i], index[1][i]] = np.median(
                img[index[0][i] - 3:index[0][i] + 4, index[1][i] - 3:index[1][i] + 4])
    ntot = n3 + n5 + n7
    sz = np.shape(img)
    m = sz[0]
    n = sz[1]
    ...
    logging.info(
        'Gamma rem: ' + str(ntot) + '  pixels substituted. ' + str(ntot * 100.0 / (m * n)) + '% of total pixels.\n'
                                                                                             'M3: ' + str(
            n3) + '=' + str(n3 * 100.0 / (m * n)) + '%;\nM5: ' + str(n5) + '=' + str(
            n5 * 100.0 / (m * n)) + '%;\nM7: ' + str(n7) + '= ' + str(n7 * 100.0 / (m * n)) + '% of total pixels.\n'
                                                                                              'Filtered image in ' + str(
            time.time() - before) + ' sec\n')

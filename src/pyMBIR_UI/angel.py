...
class AngelLauncher(QMainWindow):
    ...
    def __init__(self, parent=None):
        super(AngelLauncher, self).__init__(parent)
    ...
    def closeEvent(self, c):
        ...
        if self.gamma_help_id:
            self.gamma_help_id.close()
        if self.log_id:
            self.log_id.close()
        if self.algo_help_id:
            self.algo_help_id.close()

        o_session = SessionHandler(parent=self)
        o_session.save_from_ui()
        o_session.automatic_save()
        logging.info(" #### Leaving Angel ####")
